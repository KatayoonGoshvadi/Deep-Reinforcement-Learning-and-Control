...
class Reinforce(object):
	...
	def train(self, env, gamma=1.0):
		...
		G = np.zeros((states.shape[0],),dtype=np.float32)
		G[-1] = rewards[-1] * 0.01
		for i in range(states.shape[0]-2,-1,-1):
			G[i] = rewards[i] * 0.01 + gamma * G[i+1]

		G = torch.from_numpy(G).to(device=DEVICE)
		...
		action_probs = self.model(torch.from_numpy(states))
	...
	def test(self,env,test_ep=20,argmax_tag=True):
		...
		for ep in range(test_ep):
			...
			while not done:
				# with torch.no_grad():
				curr_state = torch.unsqueeze(torch.from_numpy(curr_state),dim=0)
...
def parse_arguments():
	...
	parser.add_argument("--use-argmax-test", dest="use_argmax_test", type=int,
						default = 1, help="using argmax during testing")
...
def main(args):
	...
	ques_path =	os.path.join(os.getcwd(),"reinforce_mod") 
