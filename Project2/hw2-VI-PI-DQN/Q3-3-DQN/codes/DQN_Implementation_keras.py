...
class QNetwork(tf.keras.Model):
	...
	def __init__(self, environment_name,obs_space,action_space, lr, save_weights_path=None):
		...
		if (environment_name=="CartPole-v0"):
			self.model.add(Dense(32,input_dim=input_dim, activation='tanh')) #30 or  50
...
class DQN_Agent():
	...
	def __init__(self, environment_name, render=False, learning_rate=1e-3 , gamma=0.99, replay_size=100000, burn_in=20000, save_weights_path=None):
...
#		go_greedy = random.random()
		...
		if(go_greedy): ## take a greedy action
	...
	def train(self, num_episodes, test_after, eval_episodes,log_path=None):
		...
		for ep in range(num_episodes+1):
			epsilon = max((1.0 - 0.95*ep/3000),0.05)
			...
			#	q_vals = self.Q_net.model.predict(np.expand_dims(curr_state,axis=0)) ## get q_values for the current state
			#	action = self.epsilon_greedy_policy(q_vals) ## take an action using epsilon greedy policy
			#	
			#	next_state, reward, done, info = self.env.step(action) ## step in the env using the chosen action
			#	self.replay_buffer
...
#				self.replay_buffer.append((curr_state,action,reward,next_state,done))
				...
				q_max_nxt_state = np.amax(self.Q_net.model.predict(sampled_nxt_states),axis=1) * (1-dones)


				for sample_id in range(q_values_target.shape[0]):
					q_val_pred = q_values_target[sample_id, int(transition_actions[sample_id])]
					q_1_step = exp_rewards[sample_id] + self.gamma * q_max_nxt_state[sample_id]
...
def main(args):
	...
	log_path = os.path.join(os.getcwd(),"keras_dqn_"+model_name)
	...
	if (args.train):
		...
		eg_weights_file = args.model_file
		test_video(agent, environment_name, vid_ep, eg_weights_file, video_save_path)
