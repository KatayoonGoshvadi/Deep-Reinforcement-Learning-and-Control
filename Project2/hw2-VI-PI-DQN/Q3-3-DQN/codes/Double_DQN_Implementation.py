...
class DQN_Agent():
	...
	def train(self,epsilon,update_target=False):
		...
		while not(done):
			...
			with torch.no_grad():
                            next_actions = torch.argmax(self.Q_net(torch.from_numpy(sampled_nxt_states)),dim=1)
                            next_actions_mask = torch.from_numpy(self.create_action_mask(next_actions.cpu().numpy())).bool().to(device=DEVICE)
                            q_max_nxt_state = torch.masked_select(self.DQ_net(torch.from_numpy(sampled_nxt_states)),next_actions_mask)
...
def parse_arguments():
	...
	parser.add_argument("--target-update-interval",dest="target_update_interval",type=int,default=100)
...
def main(args):
	...
	ques_path = os.path.join(os.getcwd(),"double_dqn") 
	env_path = os.path.join(ques_path,"env_{}".format(environment_name))
	curr_run_path = os.path.join(env_path,"num_ep_{}_lr_{}_gamma_{}_target_update_interval_{}"\
                                        .format(num_episodes,lr,gamma,target_update_interval))
	...
	if (args.train):
		...
		plt.close()
		fig = plt.figure(figsize=(16,9))
		plt.plot(mean,label="mean_cumulative_test_reward",color='darkmagenta')
		plt.fill_between(x,mean-std,mean+std,facecolor="plum",alpha=0.5)
